...
module.exports = function (request, config, isServer) {
  ...
  describe('Method GET', function () {
    ...
    it('matching parametrized request (object)', function (done) {
      request
        .get('https://domain.params.example/list')
        .query({limit: 10})
    ...
    it('matching strict parametrized request (with url)', function (done) {
      request.get('https://domain.strict-params.example/search?q=word&page=1').end(function (err, result) {
        expect(!err).toBe(true);
    ...
    it('matching strict parametrized request (mixed)', function (done) {
      request
        .get('https://domain.strict-params.example/search?q=word')
        .query({page: 1})
        .end(function (err, result) {
          expect(!err).toBe(true);
          expect(result.match.indexOf('q=word')).not.toBe(-1);
    ...
    it('unmatching strict parametrized request (missing parameter)', function (done) {
      request
        .get('https://domain.strict-params.example/search')
        .query({q: 'word'})
        .end(function (err, result) {
          expect(!err).toBe(true);
          expect(result).toBe('Real call done');
    ...
    it('matching parametrized request (no parameters)', function (done) {
      request.get('https://domain.params.example/list').end(function (err, result) {
        expect(!err).toBe(true);
        expect(result.data).toBe('Fixture !');
        done();
      });
    ...
    it('unmatching parametrized request (object)', function (done) {
      request
        .get('https://dummy.domain.params.example/list')
        .query({limit: 10})
        .end(function (err, result) {
          expect(!err).toBe(true);
          expect(result).toBe('Real call done');
    ...
    it('passing matched patterns to fixtures', function (done) {
      ...
      request.get(url).end(function (err, result) {
        ...
        expect(currentLog.warnings).toEqual([
          'This other pattern matches the query but was ignored: https://match.example/foo',
        ]);
        done();
      });
    });
    ...
    it('catches not found error and response it', function (done) {
      request.get('https://error.example/404').end(function (err, result) {
    ...
    it('catches unauthorized error and response it', function (done) {
      request.get('https://error.example/401').end(function (err, result) {
        expect(err).not.toBe(null);
    ...
    it('also can use "send" method', function (done) {
      request
        .get('https://domain.send.example/666')
        .send({superhero: 'me'})
        .end(function (err, result) {
          expect(!err).toBe(true);
          expect(result.match[1]).toBe('666');
  ...
  describe('Method POST', function () {
    ...
    it('matching parametrized request (object)', function (done) {
      request
        .post('https://domain.params.example/list')
        .query({limit: 10})
        .end(function (err, result) {
          expect(!err).toBe(true);
          expect(result.match.indexOf('limit=10')).not.toBe(-1);
    ...
    it('matching strict parametrized request (with url)', function (done) {
      request.post('https://domain.strict-params.example/search?q=word&page=1').end(function (err, result) {
        expect(!err).toBe(true);
        expect(result.match.indexOf('q=word')).not.toBe(-1);
        expect(result.match.indexOf('page=1')).not.toBe(-1);
        expect(result.data).toBe('Fixture !');
    ...
    it('matching strict parametrized request (mixed)', function (done) {
      request
        .post('https://domain.strict-params.example/search?q=word')
        .query({page: 1})
        .end(function (err, result) {
          expect(!err).toBe(true);
          expect(result.match.indexOf('q=word')).not.toBe(-1);
    ...
    it('unmatching strict parametrized request (missing parameter)', function (done) {
      request
        .post('https://domain.strict-params.example/search')
        .query({q: 'word'})
        .end(function (err, result) {
          expect(!err).toBe(true);
          expect(result).toBe('Real call done');
    ...
    it('matching parametrized request (no parameters)', function (done) {
      request.post('https://domain.params.example/list').end(function (err, result) {
        expect(!err).toBe(true);
        expect(result.data).toBe('Fixture !');
        done();
      });
    ...
    it('unmatching parametrized request (object)', function (done) {
      request
        .post('https://dummy.domain.params.example/list')
        .query({limit: 10})
        .end(function (err, result) {
          expect(!err).toBe(true);
          expect(result).toBe('Real call done');
    ...
    it('catches not found error and response it', function (done) {
      request.post('https://error.example/404').end(function (err, result) {
        expect(err).not.toBe(null);
        expect(err.status).toBe(404);
        expect(err.response).toBe(http.STATUS_CODES[404]);
        expect(result.notFound).toBeTruthy();
    ...
    it('catches validation error and response it', function (done) {
      request.post('https://validation.example').end(function (err, result) {
        expect(err).not.toBe(null);
    ...
    it('catches validation errors and sets text', function (done) {
      request.post('https://error.example/401').end(function (err, result) {
        expect(err).not.toBe(null);
        expect(err.status).toBe(401);
        expect(err.response).toBe(http.STATUS_CODES[401]);
        expect(result.unauthorized).toBeTruthy();
    ...
    it('setting multiple headers', function (done) {
      request
        .post('https://multiple-headers.example/')
  ...
  describe('Method PUT', function () {
    ...
    it('matching parametrized request (object)', function (done) {
      request
        .put('https://domain.params.example/list')
        .query({limit: 10})
        .end(function (err, result) {
          expect(!err).toBe(true);
          expect(result.match.indexOf('limit=10')).not.toBe(-1);
    ...
    it('matching strict parametrized request (with url)', function (done) {
      request.put('https://domain.strict-params.example/search?q=word&page=1').end(function (err, result) {
        expect(!err).toBe(true);
        expect(result.match.indexOf('q=word')).not.toBe(-1);
        expect(result.match.indexOf('page=1')).not.toBe(-1);
        expect(result.data).toBe('Fixture !');
    ...
    it('matching strict parametrized request (mixed)', function (done) {
      request
        .put('https://domain.strict-params.example/search?q=word')
        .query({page: 1})
        .end(function (err, result) {
          expect(!err).toBe(true);
          expect(result.match.indexOf('q=word')).not.toBe(-1);
    ...
    it('unmatching strict parametrized request (missing parameter)', function (done) {
      request
        .put('https://domain.strict-params.example/search')
        .query({q: 'word'})
        .end(function (err, result) {
          expect(!err).toBe(true);
          expect(result).toBe('Real call done');
    ...
    it('matching parametrized request (no parameters)', function (done) {
      request.put('https://domain.params.example/list').end(function (err, result) {
        expect(!err).toBe(true);
        expect(result.data).toBe('Fixture !');
        done();
      });
    ...
    it('unmatching parametrized request (object)', function (done) {
      request
        .put('https://dummy.domain.params.example/list')
        .query({limit: 10})
        .end(function (err, result) {
          expect(!err).toBe(true);
          expect(result).toBe('Real call done');
    ...
    it('passing matched patterns to fixtures', function (done) {
      var url = 'https://match.example/foo';
      request.put(url).end(function (err, result) {
        expect(result.data).toBe('foo');
        expect(currentLog.warnings).toEqual([
          'This other pattern matches the query but was ignored: https://match.example/foo',
        ]);
    ...
    it('catches not found error and response it', function (done) {
      request.put('https://error.example/404').end(function (err, result) {
        expect(err).not.toBe(null);
        expect(err.status).toBe(404);
        expect(err.response).toBe(http.STATUS_CODES[404]);
        expect(result.notFound).toBeTruthy();
    ...
    it('catches unauthorized error and response it', function (done) {
      request.put('https://error.example/401').end(function (err, result) {
        expect(err).not.toBe(null);
        expect(err.status).toBe(401);
        expect(err.response).toBe(http.STATUS_CODES[401]);
        expect(result.unauthorized).toBeTruthy();
  ...
  describe('end', function () {
    ...
    it('did not call ok() callback without status', function (done) {
      var okCalled = false;
      request
        .get('https://callback.method.example')
        .ok(function () {
          okCalled = true;
          return true;
        })
    ...
    it('not calling real api if not cancelled', function (done) {
      request.put('https://context.cancel.example/mock-call').end(function (err, result) {
        expect(!err).toBe(true);
    ...
    it('calling real api when cancelled', function (done) {
      request.put('https://context.cancel.example/real-call').end(function (err, result) {
        expect(!err).toBe(true);
        expect(result).toBe('Real call done');
        done();
      });
    ...
    it('calling callback function after specified delay', function (done) {
      request.put('https://context.delay.example/test').end(function (err, result) {
    ...
    it('calling callback function after emitting progress events', function (done) {
      var parts = 3;
      var currentPart = 1;
      var currentRequest = request.put('https://context.progress.example/' + parts).on('progress', function (e) {
