...
module.exports = function (request, config, isServer) {
  ...
  var superagentUserAgentHeader = isServer
    ? { 'User-Agent': 'node-superagent/' + superagentPackage.version }
    : {};
  ...
  beforeEach(function (go) {
    ...
    // Init module
    superagentMock = require("./../../src/superagent-mock")(
      request,
      config,
      logger
    );
  ...
  describe("Method POST", function () {
    ...
    it("matching simple request with default callback", function (done) {
      request
        .post("https://callback.method.example")
        .end(function (err, result) {
          expect(!err).toBe(true);
          expect(result.data).toBe("Fixture !");
          done();
  ...
  describe("Method PUT", function () {
    ...
    it("matching simple request with default callback", function (done) {
      request
        .put("https://callback.method.example")
        .end(function (err, result) {
          expect(!err).toBe(true);
          expect(result.data).toBe("Fixture !");
          done();
  ...
  describe('end', function () {
    it('returns request object', function (done) {
      ...
      expect(requestObject.header).toEqual(
        Object.assign({ header: "value", superagentUserAgentHeader } )
      );
      done();
    });
